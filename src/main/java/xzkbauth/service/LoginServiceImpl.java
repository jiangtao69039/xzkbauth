...
public class LoginServiceImpl {
    ...
    @Autowired
    SyUserServiceImpl syUserService;
    @Autowired
    SyWarningPlatformServiceImpl syWarningPlatformService;

    @Autowired
    SyBranchServiceImpl syBranchService;
     ...
     * @return String -> token
     */
    public CallbackResult<String> systemUserLogin(String userCod, String password, String verifyCode, String phoneCode){
        ...
        logger.info("[syUser登录操作],userCode:{},password:{},verifyCode:{},loginTimes:{}",userCod,password,verifyCode,loginTimes);
        if(loginTimes>6){
    ...
    public CallbackResult<String> check(int errorLoginTimes,String verfiyCodeInCacheKey,String errorTimesKey,SyUser user) {
        logger.info("错误次数："+errorLoginTimes);
        if(errorLoginTimes==3 || errorLoginTimes==4) {
            incrLoginErrorTimes(errorTimesKey);
            return CallbackResult.failure("验证登录次数",env.getProperty("msg.login.fulcode"),"301",getVerifyCod(verfiyCodeInCacheKey),null);
...
//			return error(300,"再输错一次账号会被锁定。");
            return CallbackResult.failure("验证登录次数",env.getProperty("msg.login.lockagin"),"301",getVerifyCod(verfiyCodeInCacheKey),null);
            //return result(301,env.getProperty("msg.login.lockagin"),getVerifyCod(verfiyCodeInCacheKey));
        }else if(errorLoginTimes>=6) {
            redisService.set(errorTimesKey, 7,1800);
            //添加告警台信息
            addWarningPlatform(user);
            return CallbackResult.failure("验证登录次数",env.getProperty("msg.login.retry"),"301",getVerifyCod(verfiyCodeInCacheKey),null);
            //return result(301,env.getProperty("msg.login.retry"),getVerifyCod(verfiyCodeInCacheKey));
        }else {
            incrLoginErrorTimes(errorTimesKey);
            return CallbackResult.failure("验证登录次数",env.getProperty("msg.login.errornamepwd"),"300",getVerifyCod(verfiyCodeInCacheKey),null);
            //return result(300,env.getProperty("msg.login.errornamepwd"),getVerifyCod(verfiyCodeInCacheKey));
        }
    }
    ...
    public void addWarningPlatform(SyUser user){
        try {
            ...
            if(syUser==null){
                ...
                logger.error(
                        "[添加告警台信息出错]--uuid:{},info:{},userCod:{}",
                        uuid,"user未找到",user.getUserCod());
                return;
            }
            SyBranch branch = syBranchService.findByBranchCod(syUser.getBranchCod());
                ...
                warningPlatform.setDetail(syUser.getUserCod()+
                        "("+ Optional.ofNullable(branch).map(SyBranch::getBranchName).orElse("") +")管理员账号锁定");
